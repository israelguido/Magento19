...
foreach ($products AS $product) {
  ...
  addCdata($prodNode->addChild('descricao'), trim($entityData['nome']));
  addCdata($prodNode->addChild('link'), $coreData['base_production_url'] . (strlen(trim($entityData['cat_url'])) > 0 ? trim($entityData['cat_url']).'/' : '') . trim($entityData['url']) . '?partner=&utm_source=buscape&utm_medium=' . trim(urlencode($entityData['nome'])));
  ...
  $preco_promo = (float) $entityData ['preco_por'];
  $valor = (!is_null($entityData ['preco_por']) ? (float) $entityData ['preco_por'] : $entityData['preco_de']);
  
  addCdata( $prodNode->addChild('preco'), number_format($valor, 2, ',', '') );

  //definicoes de parcelamento
  $parc = false;
  if ( $valor > (float) $coreData['vlr_minimo'] && (float) $coreData['vlr_minimo'] > 0 && (floor($valor / (float) $coreData['vlr_minimo']) > 1) ) 
  {
    $div = floor($valor / (float) $coreData['vlr_minimo']);
    $qtd_parcelas = ($coreData['qtd_parcelas'] > $div ? $div : $coreData['qtd_parcelas']);
    $vlr_parcela = round($valor, 2) / $qtd_parcelas;
    $parc = true;
  }
  
  if($parc) addCdata($prodNode->addChild('parcelamento'), "ou " . $qtd_parcelas . "x de " . number_format($vlr_parcela, 2, ',', '')); 

  //criando o subnode detalhes
  $detalhesNode = $prodNode->addChild('detalhes');
  addCdata ($detalhesNode->addChild ('sexo'), getAttributeOptionValue ($entityData ['sexo']));


  // Recupera as informações relativas aos produtos simples
  $query = "SELECT cor.value AS color, img.value AS image, tam.value AS size, stk.qty AS quantidade, stk.is_in_stock AS disponibilidade";
  $query.= " FROM ";
  ...
  foreach ($multiple AS $each) {
    ...
    $quantidade[] = trim($each['quantidade']);
    $disponibilidade[] = trim($each['disponibilidade']);
  }
  
  if( (is_array($colors)) && (count($colors) > 0) ) {
    $strColors = implode( ', ', array_unique($colors) );
    addCdata($detalhesNode->addChild('cor'), trim($strColors));
  }

  if (is_array($sizes) && (count($sizes) > 0)) {
    $strSizes = implode( ', ', array_unique($sizes) );
    addCdata($detalhesNode->addChild('tamanho'), trim($strSizes));
  }

  //if (is_array($quantidade) && (count($quantidade) > 0)) {
    //$strQuantidade = implode( ', ', $quantidade);
    //addCdata($detalhesNode->addChild('quantidade'), trim($strQuantidade));
  //}
  /*if( in_array( 1, $disponibilidade) ){
    addCdata($detalhesNode->addChild('disponibilidade'), trim($strDisponibilidade));
  }*/
}
...
$file = fopen($logdir . '/buscape.log', 'a+');
...
$filedir = '../temp';
if (!is_dir($logdir)) mkdir($logdir, 0777, true);
$dom->save("$filedir/buscape_".date("Y-m-d_H-i-s").".xml");
?>
