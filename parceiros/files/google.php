...
$xml = new SimpleXMLElement("<?xml version=\"1.0\" encoding=\"UTF-8\"?><rss xmlns:g=\"http://base.google.com/ns/1.0\" version=\"2.0\"></rss>", null, null, 'http://base.google.com/ns/1.0');
$ns = (object) array('g'=>'http://base.google.com/ns/1.0');
$xml->registerXPathNamespace('g', 'http://base.google.com/ns/1.0');
/*
$xml = new SimpleXMLElement("<?xml version=\"1.0\" encoding=\"UTF-8\"?><fakerss></fakerss>"); //comentar esta linha pra ficar como rss
*/
$pData = $xml->addChild('channel');
$xml->channel->addChild('title','Hering Webstore');
$xml->channel->addChild('link','http://www.heringwebstore.com.br/');
$xml->channel->addChild('description','O melhor da moda feminina, masculina, infantil, íntima e acessórios com frete grátis para todo o Brasil.');
...
function convertUtf($str){
  return trim(mb_convert_encoding($str,'UTF-8',mb_detect_encoding($str)));
}
...
foreach ($arrCore AS $val) {
  ...
  $coreData[$path] = 'http://www.heringwebstore.com.br/';
}
$coreData['base_url'] = 'http://www.heringwebstore.com.br/';
$coreData['base_media_url'] = 'http://www.heringwebstore.com.br/media/';
...
foreach ($products AS $product) {
  ...
  $prodNode->addChild('title', ucfirst(strtolower(convertUtf($entityData['nome']))));
  $prodNode->addChild('link', $coreData['base_url'] . trim($entityData['url']));
  addCdata($prodNode->addChild('description'), trim($entityData['descricao']));
  ...
  $shipping = $prodNode->addChild('g:shipping', $ns->g);
  ...
  $parc = false;
  if (
    $preco > (float) $coreData['vlr_minimo'] && 
    (float) $coreData['vlr_minimo'] > 0 &&
    (floor($preco / (float) $coreData['vlr_minimo']) > 1)
  ) {
    $div = floor($preco / (float) $coreData['vlr_minimo']);
  ...
  if (is_array($colors) && (count($colors) > 0)) {
    ...
    foreach (array_unique($colors) AS $color) {
      ...
      $cores[] = ucfirst(strtolower(convertUtf($color)));
    }
    $prodNode->addChild('g:color',implode(',', $cores), $ns->g);
  }
    $prodNode->addChild('g:google_product_category', 'Vestuário e acessórios &gt; ' . convertUtf($strCategoria), $ns->g);
    $prodNode->addChild('g:product_type','Vestuário e acessórios &gt;',$ns->g);
    $prodNode->addChild('g:brand','Hering', $ns->g);
