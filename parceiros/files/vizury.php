...
if ($coreData['base_url'] === 'http://admin.heringwebstore.com.br/')
{
...
foreach ($products AS $product) {
  ...
  $prodNode = $xml->addChild('product');

  $prodNode->addChild('pid', trim($entityData['sku']));
  addCdata($prodNode->addChild('pname'), trim($entityData['nome']));
  addCdata($prodNode->addChild('description'), trim($entityData['descricao']));
  addCdata($prodNode->addChild('category'), trim($strCategoria));
  addCdata($prodNode->addChild('oldprice'), number_format($entityData['preco_de'], 2, ',', ''));
  if(trim($entityData['preco_por']) != trim($entityData['preco_de']) && ((int) $entityData['preco_por'] > 0))
  {
      addCdata($prodNode->addChild('newprice'), number_format($entityData['preco_por'], 2, ',', ''));
  }
  else
  {
      addCdata($prodNode->addChild('newprice'), number_format($entityData['preco_de'], 2, ',', ''));
  }
  addCdata($prodNode->addChild('landingpage'), $coreData['base_url'] . trim($entityData['url']) . '?utm_source=vizury');
  addCdata($prodNode->addChild('imageurl'), $coreData['base_media_url'] . 'catalog/product' . trim($entityData['imagem']));
}
