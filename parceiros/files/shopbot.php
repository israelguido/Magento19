...
foreach ($products AS $product) {
  ...
  addCdata($prodNode->addChild('id'), trim($entityData['sku']));
  addCdata($prodNode->addChild('product_name'), trim(($entityData['nome'])));
  addCdata($prodNode->addChild('category'), trim($strCategoria));
  addCdata($prodNode->addChild('photo_url'), $coreData['base_production_url'] . 'media/catalog/product' . trim($entityData['imagem']));
  addCdata($prodNode->addChild('url'), $coreData['base_production_url'] . (strlen(trim($entityData['cat_url'])) > 0 ? trim($entityData['cat_url']).'/' : '') . trim($entityData['url']));
  ...
  addCdata ($prodNode->addChild ('original_price'), number_format ($preco, 2, ',', ''));

  addCdata($prodNode->addChild('description'), trim(($entityData['descricao'])));
  

  if($preco_promo != null)
    addCdata ($prodNode->addChild ('price'), number_format ($preco_promo, 2, ',', ''));

  //addCdata ($prodNode->addChild ('valor_promo_antigo'), number_format ($preco_promo_antigo, 2, ',', ''));
  
  addCdata ($prodNode->addChild ('brand'), 'Hering');
    addCdata ($prodNode->addChild ('gender'), getAttributeOptionValue ($entityData ['sexo']));
  ...
  if( (is_array($colors)) && (count($colors) > 0) ) {
    ...
    addCdata($prodNode->addChild('color'), trim($strColors));
  }
}
