...
foreach ($products AS $product) {
  ...
  $query = "SELECT cat.entity_id AS categoria ";
  ...
  addCdata($prodNode->addChild('image_url'), $coreData['base_production_url'] . 'media/catalog/product' . trim($entityData['imagem']));
  addCdata($prodNode->addChild('name'), trim($entityData['nome']));
  $cat = end(explode(',',$strCategoria));
  addCdata($prodNode->addChild('category'), trim($cat));
  //addCdata($prodNode->addChild('descricao'), trim($entityData['descricao']));
  
  $preco = (float) $entityData ['preco_de'];
  $preco_promo = ((float) $entityData ['preco_por'] > 0 ? (float) $entityData ['preco_por'] : $preco);
