...
$query = "SELECT category_ids FROM nostress_export WHERE searchengine = 'zoom' AND enabled = 1;";
...
//die($query);
  ...
  addCdata($prodNode->addChild('URL'), $coreData['base_url'] . trim($entityData['url']) . '?partner=&utm_source=zoom&utm_medium=' . trim(urlencode($entityData['nome'])));
  ...
  addCdata($prodNode->addChild('URL_IMAGEM'), $coreData['base_media_url'] . 'catalog/product' . trim($entityData['imagem']));
  ...
  addCdata($prodNode->addChild('NPARCELA'),$qtd_parcelas);
  addCdata($prodNode->addChild('VPARCELA'),$vlr_parcela);
...
$filedir = dirname(dirname(dirname(__FILE__))) . '/media/virtualbiz/zoom';
if (!is_dir($logdir)) mkdir($logdir, 0777, true);
$dom->save("$filedir/zoom.xml");
*/
?>
