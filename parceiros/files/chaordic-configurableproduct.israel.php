...
$connect = mysql_connect($host, $user, $pass, $base) OR die(mysql_error());
...
$categories = array( 942,943,944,945,946,947 );
$strCategories = implode(',', $categories);
$query = "SELECT DISTINCT conf.entity_id ";
$query.= "FROM ";
$query.= "  catalog_category_product AS cat INNER JOIN ";
$query.= "  catalog_product_entity AS conf ON (conf.entity_id = cat.product_id".(count($categories) > 0 ? " AND cat.category_id IN ($strCategories)" : "").") INNER JOIN ";
...
foreach ($products AS $product) {
	...
	$productData[$count]['color'] = implode(',', $colors);
	#$productData[$count]['image'] = implode(',', $colors);
	$productData[$count]['size'] = implode(',', $colors);
	#$productData[$count]['color'] = ($entityData['color']) ? number_format($entityData['preco_de'], 2, ',', '') : null;
	#$productData[$count]['preco_de'] = ($entityData['preco_de']) ? number_format($entityData['preco_de'], 2, ',', '') : null;
	#$productData[$count]['preco_de'] = ($entityData['preco_de']) ? number_format($entityData['preco_de'], 2, ',', '') : null;
	...
	$productData[$count]['color'] = $cores;
        $productData[$count]['size'] = $tamanhos;
...
//configs
$delimiter	= ';';
$filename	= 'produtos-file';
$path		= '../temp';

#gera cabeçalho
$cabecalho	= array("id_produto","link_produto", "categoria", "descricao", "preco_de", "preco_por", "cor","tamanho");
